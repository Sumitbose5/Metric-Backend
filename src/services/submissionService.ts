...
import { testCases, problemTypeSchemas, runnerTemplates, submissions, users, interviewProblems } from "../drizzle/schema";
...
async function executeSubmission(
    ...
    const verdictResult = verdict === "Pass" ? "ACCEPTED" : "WRONG_ANSWER";

    // get the user UUID using the clerkID (userId)
    ...
    // update the interview problems
    // Update the row that matches both problemId and interviewId so we mark the correct
    // interview-specific problem as solved.
    await db.update(interviewProblems).set({
        isSolved: verdictResult === "ACCEPTED" ? true : false
    }).where(
        and(
            eq(interviewProblems.problemId, problemId),
